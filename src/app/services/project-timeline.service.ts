...
export class ProjectTimelineService {
	...
	public async getProjectsTimeline(): Promise<IProjectTimelineModel[]> {
		return new Promise<IProjectTimelineModel[]>(
			(
				resolve: (projects: IProjectTimelineModel[]) => void,
				reject: (error?: HttpErrorResponse) => void
			): void => {
				resolve(this.mock);
			}
		);
		/*return new Promise<IProjectTimelineModel[]>((resolve: (projects: IProjectTimelineModel[]) => void, reject: (error?: HttpErrorResponse) => void): void => {
			this._HTTP.get(`${this._url}`)
				.subscribe((res: IProjectTimelineModel[]) => {
